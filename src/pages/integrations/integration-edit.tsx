...
export default function EditIntegrationPage() {
  ...
  useEffect(() => {
    const loadIntegration = async () => {
      if (!params.id) {
        toast.error("Integratsiya topilmadi");
        navigate("/integrations", { replace: true });
        return;
      }
        ...
        <CardContent>
          ...
          >
            <TabsList className="flex w-full">
              <TabsTrigger
                value="passport"
                className="data-[state=active]:bg-[#e4a216] data-[state=active]:text-white dark:data-[state=active]:bg-[#e4a216] dark:data-[state=active]:text-white"
              >
                Pasport
              </TabsTrigger>
              {dynamicTabs.map((tab) => (
              ...
              <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogTrigger asChild>
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className="ml-2 h-9 px-3 hover:!bg-[#f6f5e7] dark:hover:!bg-[#3a3728] hover:!text-[#e4a216] dark:hover:!text-[#e4a216]"
                  >
